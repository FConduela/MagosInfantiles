...
import { getMagoNoble } from '@/lib/api';
import fs from 'fs';
import path from 'path';

export const metadata: Metadata = {
  title: 'Mago Noble - Shows de Magia Infantil',
  description: 'Shows de magia infantil con el Mago Noble. Diversión garantizada para fiestas y eventos.',
};

export default async function NobleMagoPage() {
  const magoData = await getMagoNoble();
  // Ruta absoluta a la carpeta de imágenes del mago
  const imagesDir = path.join(process.cwd(), 'public', 'images', 'magos', 'noble-mago');
  ...
  const slides = [
    { src: '/images/magos/noble/carousel-1.jpg', alt: 'Mago Noble Show 1' },
    { src: '/images/magos/noble/carousel-2.jpg', alt: 'Mago Noble Show 2' },
    { src: '/images/magos/noble/carousel-3.jpg', alt: 'Mago Noble Show 3' },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#0A0F23]">
      ...
      <VideoSection 
        videoSrc="/images/magos/noble/video.mp4"
        ...
        <div className="bg-[#1A1F33] rounded-lg p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="relative rounded-lg overflow-hidden h-full">
              <Image
                src="/images/magos/noble/show.jpg"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {galleryImages.map((file, idx) => (
              <div key={file} className="relative h-[300px] rounded-lg overflow-hidden">
                <Image
                  src={`/images/magos/noble-mago/${file}`}
                  alt={`Galería Noble Mago ${idx + 1}`}
                  fill
