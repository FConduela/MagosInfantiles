...
import { getMagoKaiser } from '@/lib/api';
import fs from 'fs';
import path from 'path';

export const metadata: Metadata = {
  title: 'Mago Kaiser - Shows de Magia Infantil',
  description: 'Shows de magia infantil con el Mago Kaiser. Diversión garantizada para fiestas y eventos.',
};

export default async function KaiserPage() {
  const magoData = await getMagoKaiser();
  // Ruta absoluta a la carpeta de imágenes del mago
  const imagesDir = path.join(process.cwd(), 'public', 'images', 'magos', 'kaiser');
  ...
  const slides = [
    { src: '/images/magos/kaiser/carousel-1.jpg', alt: 'Mago Kaiser Show 1' },
    { src: '/images/magos/kaiser/carousel-2.jpg', alt: 'Mago Kaiser Show 2' },
    { src: '/images/magos/kaiser/carousel-3.jpg', alt: 'Mago Kaiser Show 3' },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#0A0F23]">
      ...
      <VideoSection 
        videoSrc="/images/magos/kaiser/video.mp4"
        ...
        <div className="bg-[#1A1F33] rounded-lg p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="relative rounded-lg overflow-hidden h-full">
              <Image
                src="/images/magos/kaiser/show.jpg"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {galleryImages.map((file, idx) => (
              <div key={file} className="relative h-[300px] rounded-lg overflow-hidden">
                <Image
                  src={`/images/magos/kaiser/${file}`}
                  alt={`Galería Kaiser ${idx + 1}`}
                  fill
