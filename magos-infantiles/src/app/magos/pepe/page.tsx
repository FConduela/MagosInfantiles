...
import { getMagoPepe } from '@/lib/api';
import fs from 'fs';
import path from 'path';

export const metadata: Metadata = {
  title: 'Mago Pepe - Shows de Magia Infantil',
  description: 'Shows de magia infantil con el Mago Pepe. Diversión garantizada para fiestas y eventos.',
};

export default async function PepePage() {
  const magoData = await getMagoPepe();
  // Ruta absoluta a la carpeta de imágenes del mago
  const imagesDir = path.join(process.cwd(), 'public', 'images', 'magos', 'pepe');
  ...
  const slides = [
    { src: '/images/magos/pepe/carousel-1.jpg', alt: 'Mago Pepe Show 1' },
    { src: '/images/magos/pepe/carousel-2.jpg', alt: 'Mago Pepe Show 2' },
    { src: '/images/magos/pepe/carousel-3.jpg', alt: 'Mago Pepe Show 3' },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#0A0F23]">
      ...
      <VideoSection 
        videoSrc="/images/magos/pepe/video.mp4"
        ...
        <div className="bg-[#1A1F33] rounded-lg p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="relative rounded-lg overflow-hidden h-full">
              <Image
                src="/images/magos/pepe/show.jpg"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {galleryImages.map((file, idx) => (
              <div key={file} className="relative h-[300px] rounded-lg overflow-hidden">
                <Image
                  src={`/images/magos/pepe/${file}`}
                  alt={`Galería Pepe ${idx + 1}`}
                  fill
