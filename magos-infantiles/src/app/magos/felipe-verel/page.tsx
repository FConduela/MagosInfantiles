...
import { getMagoFelipeVerel } from '@/lib/api';
import fs from 'fs';
import path from 'path';

export const metadata: Metadata = {
  title: 'Mago Felipe Verel - Shows de Magia Infantil',
  description: 'Shows de magia infantil con el Mago Felipe Verel. Diversión garantizada para fiestas y eventos.',
};

export default async function FelipeVerelPage() {
  const magoData = await getMagoFelipeVerel();
  // Ruta absoluta a la carpeta de imágenes del mago
  const imagesDir = path.join(process.cwd(), 'public', 'images', 'magos', 'felipe-verel');
  ...
  const slides = [
    { src: '/images/magos/felipe-verel/carousel-1.jpg', alt: 'Mago Felipe Verel Show 1' },
    { src: '/images/magos/felipe-verel/carousel-2.jpg', alt: 'Mago Felipe Verel Show 2' },
    { src: '/images/magos/felipe-verel/carousel-3.jpg', alt: 'Mago Felipe Verel Show 3' },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#0A0F23]">
      ...
      <VideoSection 
        videoSrc="/images/magos/felipe-verel/video.mp4"
        ...
        <div className="bg-[#1A1F33] rounded-lg p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="relative rounded-lg overflow-hidden h-full">
              <Image
                src="/images/magos/felipe-verel/show.jpg"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {galleryImages.map((file, idx) => (
              <div key={file} className="relative h-[300px] rounded-lg overflow-hidden">
                <Image
                  src={`/images/magos/felipe-verel/${file}`}
                  alt={`Galería Felipe Verel ${idx + 1}`}
                  fill
