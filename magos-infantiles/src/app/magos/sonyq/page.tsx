...
import { getMagoSonyq } from '@/lib/api';
import fs from 'fs';
import path from 'path';
import sizeOf from 'image-size';

export const metadata: Metadata = {
  title: 'Mago Sonyq - Shows de Magia Infantil',
  description: 'Shows de magia infantil con el Mago Sonyq. Diversión garantizada para fiestas y eventos.',
};

export default async function SonyqPage() {
  const magoData = await getMagoSonyq();
  // Ruta absoluta a la carpeta de imágenes del mago
  const imagesDir = path.join(process.cwd(), 'public', 'images', 'magos', 'sonyq-2025');
    ...
    galleryImages = fs.readdirSync(imagesDir)
      ...
      .filter(file => {
        try {
          ...
          return dimensions.width && dimensions.height && dimensions.width > 0 && dimensions.height > 0;
        } catch {
          return false;
        }
      })
  ...
  const slides = [
    { src: '/images/magos/sonyq/carousel-1.jpg', alt: 'Mago Sonyq Show 1' },
    { src: '/images/magos/sonyq/carousel-2.jpg', alt: 'Mago Sonyq Show 2' },
    { src: '/images/magos/sonyq/carousel-3.jpg', alt: 'Mago Sonyq Show 3' },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#0A0F23]">
      ...
      <VideoSection 
        videoSrc="/images/magos/sonyq/video.mp4"
        ...
        <div className="bg-[#1A1F33] rounded-lg p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="relative rounded-lg overflow-hidden h-full">
              <Image
                src="/images/magos/sonyq/show.jpg"
      ...
      <section className="mb-16">
        <div className="rounded-lg p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 justify-center">
            {galleryImages.slice(0, 3).map((file, idx) => (
              <div key={file} className="relative h-[300px] rounded-lg overflow-hidden">
                <Image
                  src={`/images/magos/sonyq-2025/${file}`}
                  alt={`Galería Sonyq ${idx + 1}`}
                  fill
